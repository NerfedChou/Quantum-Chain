...
impl fmt::Display for StorageError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            StorageError::InvalidBlockHash => {
                write!(f, "Invalid block hash: cannot be zero")
            }
        }
    }
}
...
use crate::domain::storage::security::StorageSecurityError;

impl From<StorageSecurityError> for StorageError {
    fn from(err: StorageSecurityError) -> Self {
        match err {
            StorageSecurityError::ChecksumMismatch {
                block_hash,
                expected,
                actual,
            } => StorageError::DataCorruption {
            ...
            StorageSecurityError::GenesisModificationAttempt { .. } => StorageError::GenesisImmutable,
            StorageSecurityError::FinalizationRegression { current, attempted } => {
                StorageError::InvalidFinalization {
                    requested: attempted,
                    current,
                }
            }
            StorageSecurityError::BlockSizeExceeded { size, max } => StorageError::BlockTooLarge {
                size,
                max_size: max,
            },
            StorageSecurityError::InvalidHash => StorageError::InvalidBlockHash,
        }
    }
}
...
impl From<KVStoreError> for StorageError {
    fn from(err: KVStoreError) -> Self {
        match err {
            KVStoreError::NotFound => StorageError::DatabaseError {
                message: "Key not found".to_string(),
            },
            _ => StorageError::DatabaseError {
