...
impl QuicTransport {
    ...
    pub async fn bind(&mut self) -> Result<SocketAddr, QuicError> {
        ...
        // Build endpoint
        let mut endpoint = quinn::Endpoint::server(server_config, self.config.bind_addr)
            .map_err(|e| QuicError::BindFailed {
    ...
    pub async fn connect(
        ...
        let connecting = endpoint.connect(remote, server_name).map_err(|_| {
    ...
    pub async fn send(&mut self, remote: SocketAddr, data: &[u8]) -> Result<(), QuicError> {
        ...
        let mut stream = connection.open_uni().await.map_err(|e| QuicError::StreamError {
            reason: e.to_string(),
        })?;
...
mod tests {
    ...
    fn test_connection_state_creation() {
        let state = QuicConnectionState::new(
