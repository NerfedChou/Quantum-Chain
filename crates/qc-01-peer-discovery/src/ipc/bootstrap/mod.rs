...
/// Configuration for creating a BootstrapRequest
pub struct BootstrapRequestConfig {
    /// 32-byte node ID
    pub node_id: [u8; 32],
    ...
    /// Proof of work nonce
    pub proof_of_work: [u8; 32],
    /// Public key claimed by the node
    pub claimed_pubkey: [u8; 33],
...
impl BootstrapRequest {
    ...
    #[must_use]
    pub fn new(config: BootstrapRequestConfig) -> Self {
        Self {
            node_id: config.node_id,
            ip_address: config.ip_address,
            port: config.port,
            proof_of_work: config.proof_of_work,
            claimed_pubkey: config.claimed_pubkey,
